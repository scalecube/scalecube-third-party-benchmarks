...
public class LmdbStorageAgronaBuffers implements Storage<UUID, Order> {
  ...
  private static final String DB_NAME = "LmdbStorageAgronaBuffers";

  private Env<DirectBuffer> env;
  private Dbi<DirectBuffer> db;

  LmdbStorageAgronaBuffers(BenchmarkSettings settings) {}
    ...
    try (Txn<DirectBuffer> txn = env.txnWrite()) {
      try (Cursor<DirectBuffer> cursor = db.openCursor(txn)) {
        ...
        MutableDirectBuffer keyBuffer =
            new UnsafeBuffer(ByteBuffer.allocateDirect(keyBytes.length));
        keyBuffer.putBytes(0, keyBytes);

        byte[] valBytes = val.toBytes();
        MutableDirectBuffer valBuffer =
            new UnsafeBuffer(ByteBuffer.allocateDirect(valBytes.length));
